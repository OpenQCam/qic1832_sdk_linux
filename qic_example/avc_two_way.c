...
if(frame_count==400){
//	qic_change_V4L2_flip(DEV_ID_1, 0x00) ;
	}
...
/*V4L2 control testing */
          ...
          if( frame_count <= 300 && test==0 )
			...
			ret=qic_change_V4L2_SATURATION( DEV_ID_1, (test_value+32)%63);	
			 ...
			 ret=qic_change_V4L2_flip(DEV_ID_0+DEV_ID_1, 0x03) ;
			 if (ret) {
                      printf("S3-4 error\n");
                  }
			 ret=qic_change_V4L2_flip(DEV_ID_0+DEV_ID_1, 0x01) ;
			 if (ret) {
                      printf("S3-5 error\n");
			 	}
		 ...
		 if(test_value==300){
		qic_change_LED_Mode(1);
		}
		if(test_value==600){
		qic_change_LED_Mode(0);
		}

		 if(test_value==300){
		qic_change_mirror_mode(DEV_ID_0+DEV_ID_1,0);
		}
		if(test_value==600){

		qic_change_mirror_mode(DEV_ID_0,1);
		}
		 if(test_value==900){
		qic_change_mirror_mode(DEV_ID_1,1);
		}
#endif
