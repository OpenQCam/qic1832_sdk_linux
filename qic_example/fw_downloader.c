 ...
 *************************************************/
	...
	if(backup_dir){
		...
		ret= qic_backup_firmware_to_file(DEV_ID_0, NULL, 0, backup_dir, USB_FIRMWARE|ISP_FIRMWARE|PARAM_FIRMWARE|AUDIO_FIRMWARE,hw_version);
#else
		ret= qic_backup_firmware_to_file(DEV_ID_0, NULL, 0, backup_dir, USB_FIRMWARE|ISP_FIRMWARE|PARAM_FIRMWARE,hw_version);
#endif
	if(!ret)
		   ...
		   QicGet1822HWVersion(&qic1822_version);
            ...
            printf("\n \033[1;33m QIC1802 version\033[m \n");
		     ...
		     printf("\n \033[1;33m unknow HW version\033[m \n");
				...
				printf("\n \033[1;33m QIC(system)HW:%.5lX VID:0x%.4lX PID:0x%.4lX REV:0x%.4lX svn(%d), fw_api_version(%d), update dir=%s \033[m \n",qic_info.hw_version, qic_info.vid, qic_info.pid, 
	...
	if (read_file_version && (usb_bin != NULL)) {
		...
		printf("\n \033[1;33m QIC(file)HW:%.5lX VID:0x%.4lX PID:0x%.4lX REV:0x%.4lX svn(%d), fw_api_version(%d), update dir=%s \033[m \n",qic_info.hw_version, qic_info.vid, qic_info.pid, 
			qic_info.revision,qic_info.svn,qic_info.fw_api_version,usb_bin);
	...
	if(BL2nd_bin){
		...
		if(have_to_erased_all){
			...
			ret= qic_backup_firmware_to_file(DEV_ID_0, NULL, 0, ".", AUDIO_FIRMWARE,hw_version);
...
#if 0//def QIC_SUPPORT_AUDIO_CALIBRATION_BACKUP
	if(audio_bin){
	...
	clock_gettime(0,&start_ts);
		...
		qic_set_lock_steam_control( 1);	  
	      ret=qic_update_firmware_by_filename(DEV_ID_0, usb_bin,NULL,isp_param_bin,audio_bin,osd_font_bin,update_flags,hw_version);
		  qic_set_lock_steam_control( 0);
		  g_runing=0;
